...
def evaluate_finance_methods():
    ...
    stock_counts = stock_counts.get_subembed(set(eval_words).union(positive_seeds).union(negative_seeds),
                                             restrict_context=False)
...
def evaluate_twitter_methods():
    ...
    embed = create_representation("GIGA", constants.TWITTER_EMBEDDINGS,
                                  set(lexicon.keys()).union(positive_seeds).union(negative_seeds))
...
def run_method(positive_seeds, negative_seeds, embeddings, transform_embeddings=False, post_densify=False,
    ...
    if transform_embeddings:
        ...
        embeddings = embedding_transformer.apply_embedding_transformation(embeddings, positive_seeds, negative_seeds,
                                                                          n_dim=50)
    if post_densify:
